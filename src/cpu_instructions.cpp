...
namespace nes
{
	...
	void cpu::add(uint8_t value)
	{
		...
		if (result > 0xFF) state.set_flags(flags::Carry);
		if (~(state.a ^ value) & (state.a ^ result) & 0x80) state.set_flags(flags::Overflow);
	...
	uint8_t cpu::shift_left(uint8_t value)
	{
		...
		if (value & 0x80) state.set_flags(flags::Carry);
		return result;
	}
	...
	uint8_t cpu::shift_right(uint8_t value)
	{
		...
		state.update_nz(result);
		state.clear_flags(flags::Carry);
		if (value & 0x01) state.set_flags(flags::Carry);
		return result;
	}

	uint8_t cpu::rotate_left(uint8_t value)
	{
		uint8_t result = (value << 1) | state.check_flags(flags::Carry);
		state.update_nz(result);
		state.clear_flags(flags::Carry);
		if (value & 0x80) state.set_flags(flags::Carry);
		return result;
	...
	void cpu::compare(uint8_t reg, uint8_t value)
	{
		...
		state.clear_flags(flags::Carry);
		if (reg >= value) state.set_flags(flags::Carry);
	}
	...
	void cpu::branch(bool taken)
	{
		...
		if (taken)
		{
			if (crosses_page(state.pc, offset)) tick();
			tick();
	...
	void cpu::INC()
	{
		auto addr  = get_operand<Mode>();
		auto value = memory_read(addr);
		if constexpr (Mode == AbsoluteX_Exception) tick();
		uint8_t result = value + 1;
		state.update_nz(result);
		tick();
	...
	void cpu::BIT()
	{
		...
		if (!(state.a & value)) state.set_flags(flags::Zero);
		if (value & 0x40) state.set_flags(flags::Overflow);
		if (value & 0x80) state.set_flags(flags::Negative);
	}
	...
	void cpu::DCP()
	{
		...
		tick();
		state.clear_flags(flags::Carry);
		if (state.a >= value) state.set_flags(flags::Carry);
	...
	uint16_t cpu::get_operand<AbsoluteX>()
	{
		...
		if (crosses_page(base_addr, state.x)) tick();
	...
	{
		auto base_addr = get_operand<Absolute>();
		if (crosses_page(base_addr, state.y)) tick();
