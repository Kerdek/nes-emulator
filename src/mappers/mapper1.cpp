...
namespace nes
{
  ...
  void mapper1::apply()
  {
    ...
    if (control & 0b1000) // 16KB PRG-ROM
    {
      if (control & 0b100) // Switchable or fixed to the first bank
      {
        set_prg_map<16>(0, prg_bank & 0xF);
        set_prg_map<16>(1, 0xF);
      }
      else // Fixed to the last bank or switchable
      {
        set_prg_map<16>(0, 0);
    ...
    if (control & 0b10000) // 4KB CHR-ROM
    {
  ...
  void mapper1::chr_write(uint16_t addr, uint8_t value)
  {
    chr[addr] = value;
  }
}
